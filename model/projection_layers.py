...
from model.layers import BatchNormRelu
...
class LinearLayer(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        # Add a dense layer, can add a bias term
        self.dense = tf.keras.layers.Dense(
            num_classes,
            kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.01),
            use_bias=use_bias and not self.use_bn)
        # if use_bn, add the batch normalization layer without relu, to ensure it is linear
        if self.use_bn:
...
class ProjectionHead(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        # if the projection head is linear, then we add one linear layer without bias and relu, and add a bath normalization layer without relu
        elif self.proj_head_mode == 'linear':
            self.linear_layers = [
                LinearLayer(
                    num_classes=out_dim, use_bias=False, use_bn=True, name='l_0')
            ]
        # For the nonlinear, we add some linear layers with bias and relu
        elif self.proj_head_mode == 'nonlinear':
            for j in range(num_proj_layers):
                if j != num_proj_layers - 1:
                    # for the middle layers, use bias and relu for the output.
                    # but in here, it only adds bias term
        ...
        if self.proj_head_mode == 'linear':
            ...
            for j in range(self.num_proj_layers):
                ...
                if j != self.num_proj_layers - 1:
                    # Here, it adds relu term for the middle of the layers
