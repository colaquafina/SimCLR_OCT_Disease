# there are two models in this file, one is a simple resnet model, the other one is the self-supervised model

import tensorflow as tf
from model.resnet_simclr import resnet
from model.projection_layers import ProjectionHead, SupervisedHead
from model.loss import add_contrastive_loss,add_supervised_loss
from dataset.data_process import batch_random_blur
...
class Pretrained_SimCLR_Model(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # GradientTape is used to calculate the gradient
        with tf.GradientTape() as tape:
            projection_head_outputs, supervised_head_outputs = self.claa(X, training=True)
            loss = None
